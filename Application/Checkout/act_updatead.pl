...
// ---------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  PhotoSQL = "SELECT COUNT( uIndex ) As NumberOfRecords FROM galleryphotos WHERE photonumber='6' AND advertid='" & ListingId & "' AND customerid='" & Var_UserId & "'"
             Call FetchData( PhotoSQL, PhotoRs, ConnTemp )
			 
  PhotoCount = PhotoRs("NumberOfRecords")
  
  If PhotoCount = "1" AND Instr(PhotoValue6, "nopicsmall.png" ) = "0" Then
  
    UdSQL = "UPDATE galleryphotos SET " & _
	        "photo='" & Photo6Value & "', " & _
			"isvertical='" & Photo6IsVert & "'" & _
	        "WHERE photonumber='6' AND advertid='" & ListingId & "' AND customerid='" & Var_UserId & "'"
			Call SaveRecord( UdSQL, UdRs, ConnTemp )
			
  ElseIf PhotoCount = "0" AND Instr(PhotoValue6, "nopicsmall.png" ) = "0" Then
  
    PhotoId6 = Sha1( Timer() & Rnd() )
