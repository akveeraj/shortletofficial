...
// ' --------------------------------------------------------------------------------------------------------------------------------------
  ...
  'ImgSQL = "SELECT COUNT(uIndex) AS NumberOfRecords FROM shortlets WHERE advertid='" & AdvertId & "'"
	...
	'If ImgCount > "0" Then
	  ...
	  'ImgSQL = "SELECT * FROM advertid WHERE advertid='" & AdvertId & "'"
		'         Call FetchData( ImgSQL, ImgRs, ConnTemp )
  ...
  If FileDoesExist = 1 Then
	  ...
	  'Set Img  = Server.CreateObject("persits.jpeg")
		...
		'	If Leased = 1 Then
			  ...
			  'Jpeg.Canvas.DrawPng 0, 0, LeasedMark
		'	End If
