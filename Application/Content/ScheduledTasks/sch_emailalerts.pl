...
// ---------------------------------------------------------------------------------------------------------
  ...
  HostName      = Request.ServerVariables("HTTP_HOST")
  AmendUrl      = "http://" & HostName & "/amend/subscriptions/?data:"  & ActionData
  CanelUrl      = "http://" & HostName & "/cancel/subscriptions/?data:" & ActionData
  NewAmendUrl   = ShortUrl( AmendUrl )
  NewCancelUrl  = ShortUrl( CancelUrl )
  ...
  EmailBody   = "Please find below our list of properties based on " & _
			    ...
			    PropertyList & "<br/>" & _
			    "<a href='" & NewAmendUrl  & "'><b>Click here</b></a> to amend your Property Alert Subscription Requirements<br/>" & _
			    "<a href='" & NewCancelUrl & "'><b>Click here</b></a> to cancel your Property Alert Subscription."
...
// ---------------------------------------------------------------------------------------------------------
 ...
 ' If Proceed = 1 Then
  ...
  SQL = "INSERT INTO scheduledtask_log " & _
		...
		") " & _
		"VALUES(" & _
		"'" & Label & " - " & SubCount & " Expiry Alerts Sent', " & _
		"'" & Var_DateStamp & "', " & _
		"'" & IpAddress     & "'  " & _
		")"
